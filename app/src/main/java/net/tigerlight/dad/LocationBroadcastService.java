...
import net.tigerlight.dad.registration.fragment.DadContactFragment;
...
import net.tigerlight.dad.webservices.CallUpdateLocation;
...
//    public static String TEST_UUID = "E2C56DB5DFFB48D2B060D0F5A71096E0";
    ...
    private void handleScanResult(ScanResult result) {
        ...
        DadContactFragment.TEST_UUID = DadContactFragment.TEST_UUID.toLowerCase();
        DadContactFragment.TEST_UUID_PREVIOUS = DadContactFragment.TEST_UUID_PREVIOUS.toLowerCase();
        ...
        if (UUIDHex.equalsIgnoreCase(Constants.NEW_UUID)) {
            ...
            if (getActivity() != null) //TODO:  Band-aid (per Rod) for unknown NPE
            {
                ...
                getActivity().startActivity(new Intent(getActivity(), DadMainActivity.class));
            } else {
...
//            Preference.getInstance().savePreferenceData(Constant.C_CODE, cName);
    ...
    public void callLocationUpdateService(double latitude, double longitude) {
        if (DadUtils.isInternetConnected(this)) {
...
//            Toast.makeText(this, "Internet is not connected", Toast.LENGTH_SHORT).show();
    ...
    private class AsyncTaskUpdateLocation extends AsyncTask<Void, Void, Void> {
        private CallUpdateLocation callUpdateLocation;
        double latitude;
        double longitude;
        ...
        protected Void doInBackground(Void... params) {
            callUpdateLocation = new CallUpdateLocation(LocationBroadcastService.this);
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (!isCancelled()) {
                if (callUpdateLocation.isSuccess()) {
                    Log.d("@@@", "" + latitude + "long:" + longitude + "userid:" + Preference.getInstance().mSharedPreferences.getString(DadConstant.USER_ID, ""));
...
//            callTestModeService();
    ...
    public void sendPushNotification() {
        if (Preference.getInstance().mSharedPreferences.getBoolean(DadConstant.IS_TEST_MODE, false)) {
            ...
            if (Preference.getInstance().mSharedPreferences.getBoolean(DadConstant.IS_ON_SETTING, false)) {
                return;
            }
            ...
            Preference.getInstance().savePreferenceData(DadConstant.LAST_LOG_TIME, "" + System.currentTimeMillis());
...
//            new PushForCrowdAlert().start();
    ...
    private void callTestModeService() {
        if (DadUtils.isInternetConnected((Activity) getApplicationContext())) {
            ...
            DadUtils.displayDialogNormalMessage(getString(R.string.app_name), getString(R.string.TAG_INTERNET_AVAILABILITY), (Activity) getApplicationContext());
        }
    ...
    private class AsyncTaskTestMode extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            callDADTest = new CallDADTest((Activity) getApplicationContext());
        }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (!isCancelled()) {
                if (callDADTest.isSuccess()) {
                    ...
                    DadUtils.displayDialog((Activity) getApplicationContext(), getString(R.string.app_name), getString(R.string.TAG_TEST_MODE_ON), getString(R.string.ok), "", false, false);
                } else {
    ...
    private boolean isAMinuteOver() {
        ...
        String lastLoggedTimeString = Preference.getInstance().mSharedPreferences.getString(DadConstant.LAST_LOG_TIME, "");
...
//        private ProgressDialog progressDialog;
        ...
        @Override
        protected Void doInBackground(Void... params) {
            callSendDanger = new CallSendDanger(getApplicationContext());
            callSendDanger.executeService(Double.parseDouble(lat), Double.parseDouble(log), "", accuracy);
            playAlarmSound();
            return null;
        }
...
//            progressDialog.setCancelable(false);
        ...
        @Override
        protected Void doInBackground(Void... params) {
            callDADTest = new CallDADTest(getApplicationContext());
            callDADTest.executeService();
            return null;
        }
