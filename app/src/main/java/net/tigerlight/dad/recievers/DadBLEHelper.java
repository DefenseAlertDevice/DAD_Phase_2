...
import net.tigerlight.dad.LocationBroadcastService;
...
public class DadBLEHelper {

    private BluetoothAdapter.LeScanCallback mLeScanCallback;
    // private LeDeviceListAdapter mLeDeviceListAdapter;
    private final String TAG = DadBLEHelper.class.getName();
    ...
    @SuppressLint("NewApi")
    public DadBLEHelper(final BaseFragment settingScreen, final boolean isFromSetting) {
        ...
        mLeScanCallback = (device, rssi, scanRecord) -> settingScreen.getActivity().runOnUiThread(() -> {
            ...
            if (msg.contains(DadContactFragment.TEST_UUID)) {
                if (isFromSetting) {
//                                restartActivity();
                } else {
                    ((DadContactFragment) settingScreen).sendPushNotification();
                }
            }

            if (msg.contains(DadContactFragment.TEST_UUID_PREVIOUS)) {
                if (isFromSetting) {
//                                restartActivity();
//                                ((SettingScreen) settingScreen).bleFound();
    ...
    @SuppressLint("NewApi")
    public DadBLEHelper(final MyBleService myBleService) {

        if (ActivityCompat.checkSelfPermission(myBleService, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
            // Permission is not granted, handle accordingly
            return;
        }
        mLeScanCallback = (device, rssi, scanRecord) -> {
            ...
            if (UUIDHex.equalsIgnoreCase(Constants.NEW_UUID)) {
                ...
                if (msg.contains(DadContactFragment.TEST_UUID) || msg.contains(DadContactFragment.TEST_UUID_PREVIOUS)) {
                    myBleService.sendPushNotification();
                    //Log.d("ss", "2");
    ...
    @SuppressLint("NewApi")
    public DadBLEHelper(final LocationBroadcastService LocationbleService, final boolean isSetting) {
