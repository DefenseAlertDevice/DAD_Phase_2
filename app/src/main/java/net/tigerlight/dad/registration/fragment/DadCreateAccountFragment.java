...
import net.tigerlight.dad.registration.webservices.CallRegister;
 ...
 */
public class DadCreateAccountFragment extends BaseFragment {
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == tvSave.getId()) {
            ...
            if (activity instanceof DadMainActivity) {
                ((DadMainActivity) activity).replaceFragment(new DadRegistrationFragment());
            }
    ...
    private void validateFragment() {
        if (etUserName.getText().toString().trim().equalsIgnoreCase("")) {
            ...
            DadUtils.displayDialog(getActivity(), getString(R.string.app_name), getString(R.string.TAG_EMAIL_ID), getString(R.string.TAG_OK), "", false, false);
            ...
            DadUtils.displayDialog(getActivity(), getString(R.string.app_name), getString(R.string.TAG_PASSWORD_EMPTYMSG), getString(R.string.TAG_OK), "", false, false);
            etPassword.requestFocus();
        } else if (etPassword.getText().toString().trim().length() < 7) {
            DadUtils.displayDialog(getActivity(), getString(R.string.app_name), getString(R.string.TAG_PASSWORD_LENGTHMSG), getString(R.string.TAG_OK), "", false, false);
            etPassword.requestFocus();
        } else if (etRePassword.getText().toString().trim().equalsIgnoreCase("")) {
            DadUtils.displayDialog(getActivity(), getString(R.string.app_name), getString(R.string.TAG_RE_PASSWORD_EMPTYMSG), getString(R.string.TAG_OK), "", false, false);
            ...
            if (checkPassWordAndConfirmPassword(etPassword.getText().toString().trim(), etRePassword.getText().toString().trim())) {
                ...
                Activity activity = getActivity();
                if (activity != null) {
                    if (DadUtils.isOnline(activity, true)) {
                        signUp();
                    } else {
                        DadUtils.displayDialog(activity, getString(R.string.app_name), getString(R.string.TAG_INTERNET_AVAILABILITY), getString(R.string.TAG_OK), "", false, false);
                    }
                }
            } else {
                DadUtils.displayDialog(getActivity(), getString(R.string.app_name), getString(R.string.TAG_PWD_RE_PWD_EMPTYMSG), getString(R.string.TAG_OK), "", false, false);
                etPassword.requestFocus();
            }
        }
    }
    ...
    private void signUp() {
        Activity activity = getActivity();
        if (activity != null && DadUtils.isInternetAvailable(activity)) {
    ...
    private class AsyncTaskSignUp extends AsyncTask<Void, Void, Void> {
        ...
        private CallRegister wsCreateAccount;
        ...
        protected Void doInBackground(Void... voids) {
            wsCreateAccount = new CallRegister(getActivity());
        ...
        protected void onPostExecute(Void result) {
            ...
            if (!isCancelled()) {
                if (wsCreateAccount.isSuccess()) {
                    ...
                    Preference.getInstance().savePreferenceData(DadConstant.KEY_EMAIL, etEmailIdStr);
                    Preference.getInstance().savePreferenceData(DadConstant.KEY_PASSWORD, etPasswordStr);
                    preference.savePreferenceData(DadConstant.USER_ID, wsCreateAccount.getUserid());
                    preference.savePreferenceData(DadConstant.IS_FIRST_ACCOUNT, true);
                    preference.savePreferenceData(DadConstant.USER_NAME, etUserNameStr);
                    Preference.getInstance().savePreferenceData(DadConstant.IS_LOGIN, true);
                    Preference.getInstance().saveEncryptedPreferenceData(DadConstant.ACCESS_TOKEN, wsCreateAccount.getAccessToken());
                    Preference.getInstance().saveEncryptedPreferenceData(DadConstant.REFRESH_TOKEN, wsCreateAccount.getRefreshToken());
                    Preference.getInstance().mSharedPreferences.edit().putLong(DadConstant.EXPIRES_IN, wsCreateAccount.getExpiresIn()).apply();
...
//                    }
                    ...
                    if (isImageUpdated) {
                        ...
                        Activity activity = getActivity();
                        if (activity instanceof DadMainActivity) {
                            ((DadMainActivity) activity).replaceFragment(new DadDashBoardWithSwipeableFragment());
                        }
                    }
                    ...
                    if (wsCreateAccount.getMessage().contains("email already exists")) {
                        DadUtils.displayDefaultDialog(
                            ...
                            getString(R.string.TAG_RESETPASSWORD_TXT),
                                (dialog, which) -> {
                                    ...
                                    Activity activity = getActivity();
                                    if (activity instanceof DadMainActivity) {
                                        DadForgotPasswordFragment fragment = DadForgotPasswordFragment.newInstance(etEmailIdStr);
                                ...
                                (dialog, which) -> {
                                    Activity activity = getActivity();
                                    if (activity instanceof DadMainActivity) {
                                        DadLoginToYourAccountFragment fragment = DadLoginToYourAccountFragment.newInstance(etEmailIdStr, etPasswordStr);
                                        ((DadMainActivity) activity).addFragment(fragment);
                                    }
                                    dialog.dismiss();
                                },
                        ...
                        DadUtils.displayDefaultDialog(
                                ...
                                (dialog, which) -> {
                                    Activity activity = getActivity();
                                    if (activity instanceof DadMainActivity) {
                                        ((DadMainActivity) activity).addFragment(new DadRegistrationFragment());
                                    }
                                    dialog.dismiss();
                                },
                                getString(R.string.TAG_TRY_AGAIN),
    ...
    private class updateProfilePicture extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            Activity activity = getActivity();
            if (activity != null && DadUtils.isInternetConnected(activity)) {
                uploadImage.executeService(path);
            }
            return null;
        }
    ...
    private void startBackgroundThreadForBLE() {
        ...
        if (activity != null) {
            ...
            Intent intent = new Intent(activity, DadBleReceiver.class);
