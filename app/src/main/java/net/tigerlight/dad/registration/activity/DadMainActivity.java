...
import net.tigerlight.dad.registration.fragment.DadAlertDetailFragment;
import net.tigerlight.dad.registration.fragment.DADLicenseFragment;
import net.tigerlight.dad.registration.fragment.DadDashBoardWithSwipeableFragment;
...
public class DadMainActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final boolean isAccepted = Preference.getInstance().mSharedPreferences.getBoolean(DadConstant.IS_ACCEPT, false);
        boolean isLogin = Preference.getInstance().mSharedPreferences.getBoolean(DadConstant.IS_LOGIN, false);

        final Intent intent = getIntent();
        final String jsonObject = intent.getStringExtra(DadConstant.JSON_OBJECT);
        // boolean openAlertFragmentDirectly = intent.getBooleanExtra("OPEN_ALERT_FRAGMENT_DIRECTLY", false);
        Log.d("notification", "oncreate ----json object:" + intent.getStringExtra(DadConstant.JSON_OBJECT));
        if (jsonObject != null) {
            if (getSupportFragmentManager().getBackStackEntryCount() > 0) {
                Log.d("notification", "oncreate if " + intent.getStringExtra(DadConstant.JSON_OBJECT));
                // Get the Back Entry
                ...
                if (fragment != null) {
                    ...
                    if (currentFragment != null) {
                        ...
                        addFragment(new DadAlertDetailFragment(), currentFragment);
                    }
                }
            } else {
                Log.d("notification", "oncreate else " + intent.getStringExtra(DadConstant.JSON_OBJECT));
            ...
            if (isLogin) {
                //if (isAccepted) {
                replaceFragment(new DadDashBoardWithSwipeableFragment());
                final var locationServiceIntent= new Intent(this, LocationUpdateService.class);
...
//            }
                ...
                if (isAccepted) {
                    replaceFragment(new DadRegistrationFragment());
                } else {
...
//        }
        ...
        getOnBackPressedDispatcher().addCallback(this, new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                if (getSupportFragmentManager().getBackStackEntryCount() > 0) {
                    Util.getInstance().hideSoftKeyboard(DadMainActivity.this);
    ...
    protected void onNewIntent(@NonNull final Intent intent) {
        ...
        Log.d("notification", "called" + intent.getStringExtra(DadConstant.JSON_OBJECT));
        // BaseFragment fragment = (BaseFragment) getSupportFragmentManager().findFragmentById(R.id.activity_registartion_fl_container);
        //if (fragment != null) {
        final DadAlertDetailFragment alertDetailFragment = new DadAlertDetailFragment();
        String jsonObject = intent.getStringExtra(DadConstant.JSON_OBJECT);
        final Bundle bundle = new Bundle();
        bundle.putString(DadConstant.JSON_OBJECT, jsonObject);
        alertDetailFragment.setArguments(bundle);
        Log.d("notification", " ifcalled" + intent.getStringExtra(DadConstant.JSON_OBJECT));
     ...
     * @param hideFragment This parameter will take fragment name which you want to hide.
     */
