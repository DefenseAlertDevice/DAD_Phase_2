...
public class CallRegister {
    ...
    public CallRegister(final Context context) {
        ...
        if (response != null && response.trim().length() > 0) {
            try {
                ...
                if (jsonObject.length() > 0) {
                    ...
                    accessToken = jsonObject.optString(DadConstant.ACCESS_TOKEN);
                    refreshToken = jsonObject.optString(DadConstant.REFRESH_TOKEN);
                    expiresIn = jsonObject.optLong(DadConstant.EXPIRES_IN);
                    preference.savePreferenceData(DadConstant.USER_ID, userid);
